...
def _gaussian(size=3, sigma=0.25, amplitude=1, normalize=False, width=None,
    ...
    for i in range(height):
        for j in range(width):
            gauss[i][j] = amplitude * math.exp(-(math.pow((j+1-center_x)/(sigma_horz*width),2)/2 + math.pow((i+1-center_y)/(sigma_vert*height),2)/2))
    if normalize:
...
def draw_gaussian(image, point, sigma):
    ...
    size = 6*sigma+1
    g = _gaussian(size)
    g_x = [max(1, -ul[0]), min(br[0], image.shape[1]) - max(1, ul[0]) + max(1, -ul[0])]
    g_y = [max(1, -ul[1]), min(br[1], image.shape[0]) - max(1, ul[1]) + max(1, -ul[1])]
    ...
    for i in range(preds.size(0)):
        for j in range (preds.size(1)):
            ...
            if pX>1 and pX<64 and pY>1 and pY<64:
                diff = torch.FloatTensor([hm_[int(pY),int(pX)+1]-hm_[int(pY),int(pX)-1], hm_[int(pY)+1,int(pX)]-hm_[int(pY)-1,int(pX)]])
...
def shuffle_lr(parts, pairs=None):
    if pairs is None:
        pairs = [[0,16], [1,15], [2,14], [3,13], [4,12], [5,11], [6,10], [7,9], [17,26], [18,25], [19,24], [20,23], [21,22], [36,45], [37,44], [38,43], [39,42], [41,46], [40,47], [31,35], [32,34], [50,52], [49,53], [48,54], [61,63], [60,64], [67,65], [59,55], [58,56]]
